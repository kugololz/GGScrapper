...
def scrape_page_MB(url):
    try:
        ...
        data = []
        for sku, title, price, stock_status in zip(skus, titles, prices, stock):
            marca = "AMD" if "AMD" in title else "AORUS" if "AORUS" in title else "ASROCK" if "ASRock" in title else "ASUS" if "ASUS" in title else "BIOSTAR" if "Biostar" in title else "ECS" if "ECS" in title else "GIGABYTE" if "gigabyte" in title.lower() else "MSI" if "MSI" in title else "NZXT" if "NZXT" in title else ""
